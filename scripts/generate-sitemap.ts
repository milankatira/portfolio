...
// Use __filename and __dirname for CommonJS compatibility
...
async function generateSitemaps(): Promise<void> {
  ...
  const sitemapUrls: SitemapIndexEntry[] = [];
  ...
  for (let i = 0; i < numSitemaps; i++) {
    ...
    const chunk: SitemapUrlItem[] = allUrls.slice(start, end);
